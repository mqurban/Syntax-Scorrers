...
function fronts_scripts()
{
	wp_enqueue_style('bootstrap-css', plugins_url('related-posts-thumbnails/bootstrap.min.css'));
	wp_enqueue_script('file_cdn', plugins_url('related-posts-thumbnails/bootstrap.bundle.min.js'));
}

add_action('wp_enqueue_scripts', 'fronts_scripts');
...

{
		...
		foreach ($posts as $post) {
			...
			if (($amp_endpoint) && (true === apply_filters('rpth_amp', true))) {
				...
				if ($relpoststh_output_style == 'list') {
					...
					if (!$relpoststh_sliderView && !$relpoststh_mosaicView) {
	...
	{ ?>
		...
		<div class="select_specific" <?php echo esc_attr($specific_name);
										if ($categoriesall == '1') : ?> style="display:none" <?php endif; ?>>
		</div>
...
<!-- slider styles -->
<?php
$relpoststh_sliderView = get_option('relpoststh_sliderView');
$relpoststh_slider_style = get_option('rpt_slider_style');
$relpoststh_mosaicView = get_option('relpoststh_mosaicView');

// print_r($relpoststh_mosaicView);

function slider_scripts()
{
	wp_enqueue_style('slider-css', plugin_dir_url(__FILE__) . '/slider.css');
	wp_enqueue_script('slider-js', plugin_dir_url(__FILE__) . '/slider.js');
...
function add_custom_content_after_post_content($content)
{
	global $relpoststh_sliderView;
	global $relpoststh_slider_style;
	global $relpoststh_mosaicView;

	// Check if both conditions are true
	if ($relpoststh_sliderView && $relpoststh_slider_style == "style1") {
		if (is_single()) {
			// Initialize an empty string to store slider HTML
			$slider_html = '';
			...
			if ($query->have_posts()) {
				$slider_html .= '<div class="fk-slider">';
				$slider_html .= '<div class="fk-cards">';

				while ($query->have_posts()) {
					$query->the_post();
					// Generate slide HTML for each post
					$slider_html .= '<div class="fk-card-wrapper">';
					$slider_html .= '<div class="fk-card">';
					$slider_html .= '<div class="fk-img-wrapper">';
					$slider_html .= '<img src="' . get_the_post_thumbnail_url() . '" alt="' . get_the_title() . '">';
					$slider_html .= '</div>';
					$slider_html .= '<p class="fk-content-wrapper">';
					$slider_html .= '<span class="fk-title">' . get_the_title() . '</span>';
					$slider_html .= '<span class="fk-price"><a href="' . get_permalink() . '">Read More</a></span>';
					$slider_html .= '</p>';
					$slider_html .= '</div>';
					$slider_html .= '</div>';
				}
			...
			// Append slider HTML to the content
			$content .= $slider_html;
		}
	}
	return $content;
}
add_filter('the_content', 'add_custom_content_after_post_content');


function add_custom_content_after_post_content_style2($content)
{
	global $relpoststh_sliderView;
	global $relpoststh_slider_style;

	// Check if both conditions are true and style is style2
	if ($relpoststh_sliderView && $relpoststh_slider_style == "style2") {
		if (is_single()) {
			// Initialize an empty string to store slider HTML
			$slider_style = '';
			...
			if ($posts_query->have_posts()) {
				while ($posts_query->have_posts()) {
					...
					$slider_style .= '<div class="slide">';
					$slider_style .= '<h2>' . get_the_title() . '</h2>';
					$slider_style .= '<img src="' . get_the_post_thumbnail_url() . '" alt="' . get_the_title() . '">';
					$slider_style .= '<p><a href="' . get_permalink() . '">Read more</a></p>';
					$slider_style .= '</div>';
				}
			...
			// Append slider HTML to the content with additional wrapper div
			$content .= '<div class="slider-wrapper"><div class="slider">' . $slider_style . '</div></div>';
		}
	}
	return $content;
}
add_filter('the_content', 'add_custom_content_after_post_content_style2');


function add_custom_content_after_post_mosaicView($content)
{
	global $relpoststh_mosaicView;

	// Check if both conditions are true and style is style2
	if ($relpoststh_mosaicView && is_single()) {
		// Initialize an empty string to store mosaic view HTML
		$mosaic_html = '';
		...
		if ($posts_query->have_posts()) {
			...
			$mosaic_html .= '<div class="gallery">';

			while ($posts_query->have_posts()) {
				$posts_query->the_post();
				// Generate HTML for each post as an image container in the gallery
				$mosaic_html .= '<div class="image-container">';
				$mosaic_html .= '<img src="' . get_the_post_thumbnail_url(get_the_ID(), 'thumbnail') . '" alt="' . get_the_title() . '">';
				$mosaic_html .= '<div class="lightbox">';
				$mosaic_html .= '<h2>' . get_the_title() . '</h2>';
				$mosaic_html .= '<a href="' . get_permalink() . '" class="read-more">Read More</a>';
				$mosaic_html .= '</div>';
				$mosaic_html .= '</div>';
			}
		...
		// Append mosaic view HTML after the content
		$content .= $mosaic_html;
	}

	return $content;
}
add_filter('the_content', 'add_custom_content_after_post_mosaicView');
